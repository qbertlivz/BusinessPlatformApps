...
a.	In the Azure Portal, select “Resource groups” from the left pane
b.	Locate “SolutionTemplate-xxxxxxx”, the resource group we created for you
c.	Inside the resource group, navigate to “SAJob-xxxxx”, the Stream Analytics job
2.	Stop the Stream Analytics job
a.	In the “Overview” pane ensure the job is stopped, which enables editing.
3.	Add a PowerBI output
a.	In the “Outputs” pane, add a new output with the “Sink” parameter set to PowerBI.
b.	Authenticate with PowerBI
c.	Output alias: PBIOutput
i.	Note: If you use a different output alias, you will have to use that same output alias in the SELECT … INTO [<output alias here>] when defining the query in the next step
d.	Group Workspace: Select the workspace where you would like to set up the real-time streaming dataset 
e.	Dataset Name: Enter a new name for the streaming dataset. 
i.	Take note of this dataset name, as you must reference it in PowerBI when setting up real-time analytics.
f.	Table Name: Enter a new name for the table, no need to take note.
g.	Press “Save” at the bottom to save the new output
4.	Update your Stream Analytics Query
a.	Navigate to the “Query” pane. 
b.	Paste the query given below on a new line in the query window. NOTE: If you used a different output alias than “PBIOutput”, use that alias in the INTO clause at the bottom of this query:
...
c.	Press “Save” to update the query
5.	Start the Stream Analytics job
a.	In the “Overview” pane, start the Stream Analytics job. This restarts the pipeline we initially created for you, but now you are additionally sending data to a real-time data set. You are now just a few steps away from real-time analytics!
6.	Publish your existing .pbix
a.	In Power BI Desktop, open your ActivityLogTemplate.pbix that you received after completing the solution template installation wizard 
b.	In the “Home” tab press “Publish” to send the file to the web version of PowerBI 
c.	When prompted, choose the same workspace that you provided in Step 3 when setting up a Power BI output for your Stream Analytics job
7.	Pin to a dashboard
a.	After publishing from Power BI Desktop, refresh the web version of PowerBI and ensure that there is now a report listed under “Reports” on the left pane that matches the name of the Power BI Desktop file. 
b.	Select a page from this report that you would like to place next to your real-time visualization, and press “Pin Live Page” at the top.
c.	When prompted, opt to pin to a new dashboard and provide a name. 
8.	Create a report
a.	Navigate to powerbi.microsoft.com and sign in. 
b.	Expand your selected workspace, and select the dataset you created in Step 3 when setting up a Power BI output for your Stream Analytics job. 
i.	Note: The dataset will only appear if data has flowed through Stream Analytics since starting the job in Step 5. If there has been no data transmitted, the dataset will not appear. If you believe your issue is unrelated, check the troubleshooting checklist at the bottom of this guide. 
c.	Create your visual(s). In the top right corner of the visual, select the pin to pin the visual. 
d.	When prompted, provide a new name for the report 
e.	When prompted, choose to pin to an existing dashboard, the same one created in Step 7 above.
9.	Enjoy real-time streaming!

Here’s a checklist you can use to troubleshoot issues connecting Stream Analytics and Power BI: 1. Restart the Azure Stream Analytics job (jobs created before the streaming GA release will require a restart) 
