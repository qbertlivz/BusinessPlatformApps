...
-- =============================================
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA='csrv' AND TABLE_NAME='ssas_jobs' AND TABLE_TYPE='BASE TABLE')
    DROP TABLE csrv.ssas_jobs;
    
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_SCHEMA='csrv' AND ROUTINE_NAME='sp_set_process_flag' AND ROUTINE_TYPE='PROCEDURE')
    DROP PROCEDURE [csrv].sp_set_process_flag;
    
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_SCHEMA='csrv' AND ROUTINE_NAME='sp_validate_schema' AND ROUTINE_TYPE='PROCEDURE')
    DROP PROCEDURE [csrv].sp_validate_schema;
    
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_SCHEMA='csrv' AND ROUTINE_NAME='sp_get_current_record_counts' AND ROUTINE_TYPE='PROCEDURE')
    DROP PROCEDURE [csrv].sp_get_current_record_counts;
    
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_SCHEMA='csrv' AND ROUTINE_NAME='sp_check_record_counts_changed' AND ROUTINE_TYPE='PROCEDURE')
    DROP PROCEDURE [csrv].sp_check_record_counts_changed;
    
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_SCHEMA='csrv' AND ROUTINE_NAME='sp_finish_job' AND ROUTINE_TYPE='PROCEDURE')
    DROP PROCEDURE [csrv].sp_finish_job;
    
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_SCHEMA='csrv' AND ROUTINE_NAME='sp_start_job' AND ROUTINE_TYPE='PROCEDURE')
    DROP PROCEDURE [csrv].sp_start_job;
    
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_SCHEMA='csrv' AND ROUTINE_NAME='sp_reset_job' AND ROUTINE_TYPE='PROCEDURE')
    DROP PROCEDURE [csrv].sp_reset_job;
...
INSERT csrv.[configuration] (configuration_group, configuration_subgroup, [name], [value], [visible])
...
-- =============================================
-- set process flag
CREATE PROCEDURE csrv.sp_set_process_flag
...
-- get record counts
CREATE PROCEDURE csrv.sp_check_record_counts_changed AS
BEGIN
    SET NOCOUNT ON;
    DECLARE @newRowCount INT;
    DECLARE @oldRowCount INT;
    EXECUTE @newRowCount = csrv.sp_get_current_record_counts;
...
-- finish job
CREATE PROCEDURE [csrv].[sp_finish_job]
    @jobid INT,
...
-- timeout jobs
CREATE PROCEDURE csrv.sp_reset_job
AS
BEGIN
    SET NOCOUNT ON;
    UPDATE csrv.[ssas_jobs] 
    ...
    (SELECT [value] FROM csrv.[configuration] WHERE [configuration_group] = 'SolutionTemplate' AND [configuration_subgroup]='SSAS' AND [name]='Timeout')
...
-- start job
CREATE PROCEDURE csrv.sp_start_job
AS
BEGIN
    SET NOCOUNT ON;
    ...
    EXEC csrv.sp_reset_job
    
    INSERT csrv.[ssas_jobs] (startTime, statusMessage)
    ...
    if(@validateSchema = 1)
    BEGIN		
        EXECUTE @validateSchemaResult = csrv.sp_validate_schema;
    ...
    BEGIN
        ...
        EXECUTE @checkRowCountsResult = csrv.sp_check_record_counts_changed;
    ...
    IF(@checksPassed = 0)
    BEGIN
        EXEC [csrv].[sp_finish_job] @jobid= @id, @jobMessage= @errorMessage
        return 0;
    END

    EXEC [csrv].[sp_set_process_flag] @process_flag = '0'
