...
BEGIN
    ...
    WHERE
        ...
        ta.name IN ('account', 'bookableresource', 'bookableresourcebooking', 'bookingstatus', 'msdyn_customerasset','msdyn_incidenttype',
		'msdyn_workorder', 'msdyn_workorderproduct', 'msdyn_workorderservice','msdyn_workordertype','product','resource', 'service', 
		'serviceappointment','task', 'territory')
    GROUP BY ta.name
    ORDER BY ta.name;
END;
go
...
SELECT CASE
		...
		FROM #counts c INNER JOIN fso.entityinitialcount i ON i.entityname = c.entityname



	DECLARE @DeploymentTimestamp datetime2;
 	SELECT @DeploymentTimestamp = Convert(DATETIME2, [value], 126)
	FROM fso.[configuration] WHERE configuration_group = 'SolutionTemplate' AND configuration_subgroup = 'Notifier' AND [name] = 'DeploymentTimestamp';
    ...
    FROM fso.[configuration] WHERE configuration_group = 'SolutionTemplate' AND configuration_subgroup = 'Notifier' AND [name] = 'DataPullCompleteThreshold';
    ...
    FROM #percentages p
              INNER JOIN fso.entityinitialcount i ON i.entityName = p.EntityName
              WHERE 
    ...
    IF EXISTS (SELECT * FROM fso.[configuration] WHERE configuration_group = 'SolutionTemplate' AND configuration_subgroup = 'Notifier' AND [name] = 'ASDeployment' AND [value] ='true')
	SET @ASDeployment = 1;

    -- AS Flow
    IF @ASDeployment=1 AND DATEDIFF(HOUR, @DeploymentTimestamp, Sysdatetime()) < 24 AND NOT EXISTS (SELECT * FROM fso.ssas_jobs WHERE [statusMessage] = 'Success')
	SET @StatusCode = -1;

    -- Delayed Processing Flow
    DECLARE @c1 INT, @c2 INT;
    SELECT @c1 = COUNT(*) FROM #counts;
    SELECT @c2 = COUNT(*) from fso.entityinitialcount;
    IF @c1<>@c2 
    SET @StatusCode = -1;
    ...
    MERGE fso.entityinitialcount AS target
...
BEGIN
    ...
    WHERE  ( table_schema = 'dbo' AND
             table_name IN ('account', 'bookableresource', 'bookableresourcebooking', 'bookingstatus', 'msdyn_customerasset','msdyn_incidenttype',
		'msdyn_workorder', 'msdyn_workorderproduct', 'msdyn_workorderservice','msdyn_workordertype','product','resource', 'service', 
		'serviceappointment','task', 'territory')
           ) OR
           ( table_schema = 'fso' AND
             table_name IN ('BookingStatusView', 'CustomerEquipmentView', 'configuration', 'configurationview', 'CustomerView', 'date', 'dateview',
                            'incidenttypeview', 'measuresview', 'productview', 'resourcebookingview', 'resourceview',
							'territoryview', 'workorderproductsview','workorderserviceview','workordertypeview','workorderview'
                           )
           );
END;
go
